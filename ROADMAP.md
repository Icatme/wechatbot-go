...
- **现状**：Go 版本仅用 `sync.Map` 内存缓存，进程重启丢失。
...
- **现状**：每次 `SendTyping`/`StopTyping` 都调 `getConfig`。
...
- **现状**：Go 版本 `base_info` 只有 `channel_version`。
...
- **现状**：Go 固定 45s，忽略服务器返回的 `longpolling_timeout_ms`。
- **目标**：动态调整下次长轮询超时。
...
- **现状**：Go `Download()` 返回 raw SILK bytes。
...
- **现状**：Go 把 caption 和 media 塞进同一个 `item_list`。
...
- **现状**：Go 未处理 `need_verifycode` / `verify_code_blocked`。
